...
Mat draw_contour(Mat src_gray,Point array[3],int& sz,Point scale[3],Mat image,Point location[3][2],Mat &segment)
{
	...
	int thresh = 100,max_thresh = 255;
	  ...
	  for( int i = 0; i < (int) contours.size(); i++ )
	     {
		  	  ...
		  	  drawContours( drawing, contours_poly, i, colorB, -1, 8, vector<Vec4i>(), 0, Point() );
	  ...
	  for(int j=size-1;j>=0 and maxi;j--)
	  	...
	  	location[sz][0]=boundrectangle[i].second.tl();
	  	location[sz][1]=boundrectangle[i].second.br();
	  ...
	  //	convexityDefects(contours[i], hull[i], convexityDefect[i]);
	  	...
	  	for(int i=0;i< hull.size();i++)
	  	{
	  					...
	  					int n=hull[i].size();
	  		  			...
	  		  			{
	  		  				MyFilledCircle(draw,hull[i][j],0);
                            mid.x+=hull[i][j].x;
                            mid.y+=hull[i][j].y;
	  		  			...
	  		  			mid.x/=n;
	  		  			mid.y/=n;
	  		  			MyFilledCircle(draw,mid,1);

	  	}
	  	bitwise_and(drawing,drawing,temp,src_gray);
...
Mat contour(Mat src_gray,Point array[3],int& sz,Point scale[3])
{
