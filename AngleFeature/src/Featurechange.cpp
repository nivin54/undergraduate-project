...
MatrixDouble feature_extraction(vector<Point> P,UINT DIM)
{
	...
	{
		datas[0]=tanh(((P[i+1].y-cy)/(P[i+1].x-cy)));
		datas[1]=tanh((P[i+1].y-P[i].y)/(P[i+1].x-P[i].x));
		datas[2]=tanh((P[i+1].y-P[0].y)/(P[i+1].x-P[0].x));
...
int main(int argc, const char *argv[])
{
	...
	VideoCapture cap("/home/mac/Documents/PROJECT/Training_Sets/Air/AIR_1_NAVIN (1).mp4");
	noframes=1;
	Mat3b frame;
